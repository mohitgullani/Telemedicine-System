...
public class PatientLoginFrame extends javax.swing.JFrame {
    ...
    public PatientLoginFrame(String status) {
        initComponents(); 
...
Connection con;
    ...
    private void jLabel1MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabel1MouseClicked
        NewPatientDetails np=new NewPatientDetails();
        np.setVisible(true);
    ...
    private void l3MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_l3MouseClicked
        ...
        try
        {
            ...
            ResultSet rs=st.executeQuery("Select count(*) from Patient where Username='"+username+"' and Password='"+password+"' and Account_Status='Activated'");
            while(rs.next())
            {
                count=rs.getInt("count(*)");
                if(count==1){
                    ResultSet rs1=st.executeQuery("Select Patient_ID from Patient where Username='"+username+"' and Password='"+password+"'");
                    while(rs1.next())
                    {
                        pid=rs1.getInt("Patient_ID");
                        PatientDashBoard p=new PatientDashBoard(pid);
                        p.setVisible(true);
                        this.dispose();
                    }
                }else{
    ...
    private void tf1KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tf1KeyReleased
        ...
        if(code==10)
        {
            ...
            {
                Statement st=con.createStatement();
                ResultSet rs=st.executeQuery("Select count(*) from Patient where Username='"+username+"' and Password='"+password+"'");
                while(rs.next())
                {
                    count=rs.getInt("count(*)");
                    if(count==1){
