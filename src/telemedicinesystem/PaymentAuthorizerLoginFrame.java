...
public class PaymentAuthorizerLoginFrame extends javax.swing.JFrame {

    public PaymentAuthorizerLoginFrame(String status) {
        initComponents(); 
...
Connection con;
    ...
    private void l3MouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_l3MouseClicked
        ...
        try
        {
            ...
            ResultSet rs=st.executeQuery("Select count(*) from User where Username='"+username+"' and Password='"+password+"' and Account_Status='Activated'");
            while(rs.next())
            {
                count=rs.getInt("count(*)");
                if(count==1){
                    ResultSet rs1=st.executeQuery("Select User_ID from User where Username='"+username+"' and Password='"+password+"'");
                    while(rs1.next())
                    {
                        uid=rs1.getInt("User_ID");
                        PaymentAuthorizerDashBoard u=new PaymentAuthorizerDashBoard(uid);
                        u.setVisible(true);
                        this.dispose();
                    }
                }else{
    ...
    private void tf1KeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tf1KeyReleased
        ...
        if(code==10)
        {
            ...
            {
                Statement st=con.createStatement();
                ResultSet rs=st.executeQuery("Select count(*) from User where Username='"+username+"' and Password='"+password+"'");
                while(rs.next())
                {
                    count=rs.getInt("count(*)");
                    if(count==1){
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(PaymentAuthorizerLoginFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new PaymentAuthorizerLoginFrame("").setVisible(true);
            }
        });
    }
